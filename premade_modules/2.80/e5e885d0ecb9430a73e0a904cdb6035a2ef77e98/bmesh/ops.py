...
def extrude_face_region(bm, geom, edges_exclude, use_keep_orig,
                        use_normal_flip, use_select_history):
...
def bevel(bm, geom, offset, offset_type, segments, profile, vertex_only,
          clamp_overlap, material, loop_slide, mark_seam, mark_sharp, strength,
          hnmode):
    ...
    :param strength: strength of calculated normal in range (0, 1) for custom clnors 
    :type strength: float
    :param hnmode: harden normals mode used in bevel, if enabled 
    :type hnmode: enum in ['NONE', 'FACE', 'ADJACENT', 'FIXED_NORMAL_SHADING'], default 'NONE'
    :return:  faces: output facestype list of (bmesh.types.BMFace)edges: output edgestype list of (bmesh.types.BMEdge)verts: output vertstype list of (bmesh.types.BMVert)normals: output normals per vertex for beveled edgestype dict mapping vert/edge/face types to unknown internal data, not compatible with python 
    '''
